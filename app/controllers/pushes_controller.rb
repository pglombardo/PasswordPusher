...
class PushesController < BaseController
  ...
  def create
    ...
    if file_push_params.key?(:files) &&
      file_push_params[:files].count { |e| e != "" } > settings_for(push).max_file_uploads
  ...
  def destroy
    ...
    @push.payload = nil
    @push.deleted = true
    # MIGRATE - ask - in the premium one, it is destroyed immediately. Why are they marked here
    @push.files.purge
  ...
  def create_detect_retrieval_step(push, params)
    if settings_for(push).enable_retrieval_step == true
      push_params = params[:password] || params[:url] || params[:file]
  ...
  def create_detect_deletable_by_viewer(push, params)
    if settings_for(push).enable_deletable_pushes == true
      ...
      if push_params.key?(:deletable_by_viewer)
        # User form data or json API request: :deletable_by_viewer can
        # be 'on', 'true', 'checked' or 'yes' to indicate a positive
        user_dvb = push_params[:deletable_by_viewer].to_s.downcase
  ...
  def set_push
    @push = Push.includes(:views).find_by!(url_token: params[:id])
  ...
  def push_params
    params.require(:push).permit(:payload, :expire_after_days, :expire_after_views,
