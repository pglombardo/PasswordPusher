...
class PasswordJsonRetrievalTest < ActionDispatch::IntegrationTest
  def test_view_expiration
    ...
    assert res.key?("expire_after_views")
    assert_equal 2, res["expire_after_views"]
    assert_equal res.keys, ["expire_after_days", "expire_after_views", "expired", "url_token", "created_at", "updated_at", "deleted", "deletable_by_viewer", "retrieval_step", "expired_on", "payload", "days_remaining", "views_remaining"]
